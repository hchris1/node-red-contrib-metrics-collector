class NodeRedHooks {
    ...
    processBatch() {
        ...
        batch.forEach(({ nodeId, nodeName, nodeType, flowId, type }) => {
            if (this.options.enableDetailedLogging) {
                console.log(`Recording message for node ${nodeType} "${nodeName}" (${nodeId}) in flow ${flowId} - type: ${type}`);
            }

            if (type === 'send') {
                this.metricsCollector.recordOutgoingMessage(nodeId, nodeType, flowId, nodeName);
            } else if (type === 'receive') {
                this.metricsCollector.recordIncomingMessage(nodeId, nodeType, flowId, nodeName);
            }
        });
    ...
    addToBatch(nodeId, nodeType, flowId, type, nodeName = '') {
        this.messageBatch.push({ nodeId, nodeName, nodeType, flowId, type });
    ...
    handleSendEvent(sendEvent) {
        try {
            ...
            if (this.options.enableDetailedLogging) {
                console.log(`Send Event - Node: ${nodeType} "${nodeName}" (${nodeId}) in flow ${flowId}, msgId: ${messageId}`);
            }

            try {
                this.addToBatch(nodeId, nodeType, flowId, 'send', nodeName);

                const timingKey = `${nodeId}_${messageId}`;
                this.messageStartTimes.set(timingKey, Date.now());

            } catch (error) {
                this.metricsCollector.recordError(nodeId, nodeType, flowId, 'send_processing', nodeName);
                if (this.options.enableDetailedLogging) {
    ...
    handleReceiveEvent(receiveEvent) {
        try {
            ...
            if (this.options.enableDetailedLogging) {
                console.log(`Receive Event - Node: ${nodeType} "${nodeName}" (${nodeId}) in flow ${flowId}`);
            }
            ...
            try {
                ...
                this.addToBatch(nodeId, nodeType, flowId, 'receive', nodeName);

            } catch (error) {
                this.metricsCollector.recordError(nodeId, nodeType, flowId, 'receive_processing', nodeName);
                if (this.options.enableDetailedLogging) {
        ...
        try {
            ...
            try {
                ...
                if (startTime) {
                    ...
                    this.metricsCollector.recordNodeExecution(nodeId, nodeType, flowId, executionTime, nodeName);
                ...
                if (completeEvent.error) {
                    ...
                    this.metricsCollector.recordError(nodeId, nodeType, flowId, errorType, nodeName);
                ...
                this.metricsCollector.recordError(nodeId, nodeType, flowId, 'complete_processing', nodeName);
                if (this.options.enableDetailedLogging) {
    ...
    handleNodeError(event) {
        try {
            ...
            if (this.options.enableDetailedLogging) {
                console.log(`Node error recorded: ${nodeType} "${nodeName}" (${nodeId}) in flow ${flowId} - ${errorType}`);
            }
        } catch (error) {
