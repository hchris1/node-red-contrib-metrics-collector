...
class NodeRedHooks {
    ...
    handleSendEvent(sendEvent) {
        try {
            ...
            try {
                // Count messages on send events for regular processing nodes
                // Skip source nodes here since they'll be counted on receive
    ...
    handleReceiveEvent(receiveEvent) {
        try {
            ...
            try {
                // Record timing for execution measurement
                const messageId = receiveEvent.msg?._msgid || this.generateMessageId();
                const timingKey = `${nodeId}_${messageId}`;
                this.messageStartTimes.set(timingKey, Date.now());
                
                // Count messages for:
                // 1. Sink nodes (debug, email, etc.) - they only receive, never send
                // 2. Source nodes (inject, etc.) - count on receive since send events may not fire
                // 3. Skip regular processing nodes - they're counted on send
                if (this.isSinkNode(nodeType)) {
                    this.addToBatch(nodeId, nodeType, flowId, 'receive');
                } else if (this.isSourceNode(nodeType)) {
                    // Source nodes like inject are counted on receive
                    this.addToBatch(nodeId, nodeType, flowId, 'source_receive');
                }
                // Regular processing nodes are counted on send events only
    ...
    handleCompleteEvent(completeEvent) {
        try {
            ...
            try {
                // Don't count messages on complete to avoid triple counting
                // (messages are counted on send only)
    ...
    /**
     * Check if a node type is a sink/passive node (doesn't send messages)
     * @param {string} nodeType - The node type to check
     * @returns {boolean} True if it's a sink node
     */
    isSinkNode(nodeType) {
        const sinkNodeTypes = [
            'debug',           // Debug output
            'file',            // File write operations  
            'email',           // Email sending
            'http request',    // HTTP requests (when used as sink)
            'mqtt out',        // MQTT publish
            'tcp out',         // TCP output
            'udp out',         // UDP output
            'exec',            // Command execution
            'rbe',             // Report by exception (sometimes)
            'delay',           // Sometimes acts as sink
        ...
        return sinkNodeTypes.includes(nodeType);
    }

    /**
     * Check if a node type is a source node (generates messages internally)
     * @param {string} nodeType - The node type to check
     * @returns {boolean} True if it's a source node
     */
    isSourceNode(nodeType) {
        const sourceNodeTypes = [
            'inject',          // Manual/timer injection
            'mqtt in',         // MQTT subscription
            'tcp in',          // TCP input
            'udp in',          // UDP input
            'http in',         // HTTP endpoints
            'websocket in',    // WebSocket input
            'serial in',       // Serial input
            'file in',         // File watch/read
            'watch',           // File system watch
            'rpi-gpio in',     // GPIO input
            'trigger'          // Trigger node
        ];
        
        return sourceNodeTypes.includes(nodeType);
    }
